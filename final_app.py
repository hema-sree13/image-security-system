...
<style>
    ...
    .stTabs [data-baseweb="tab"] { color: #e8edf5; }
    .stTabs [data-baseweb="tab-list"] { background-color: #111827; }
</style>
""", unsafe_allow_html=True)

st.markdown("# 🛡️ Image Security System")
st.markdown("**Complete protection — Detect fakes + Watermark + Report**")
st.markdown("---")
...
# ─────────────────────────────────────────────────────────────────────────────
with tab1:
    st.markdown("### 🔍 Detect if an image is fake or real")
    uploaded = st.file_uploader("Upload image to analyze", type=['jpg','jpeg','png','bmp','webp'], key="detect")

    if uploaded:
        image = Image.open(uploaded)
        st.image(image, caption=f"File: {uploaded.name}", use_container_width=True)
        st.markdown(f"**Size:** {image.size[0]} x {image.size[1]} | **File:** {uploaded.name}")
        st.markdown("---")
        ...
        with col2:
            st.image(results['ela_image'], caption="ELA Map (bright = suspicious)", use_container_width=True)
        ...
        # Report button
        st.markdown("---")
        st.markdown("### 🚨 Report this image as abuse")
        reporter_name = st.text_input("Your name (optional)", key="r_name")
        report_reason = st.text_area("Why are you reporting this image?", key="r_reason")
        ...
        if st.button("🚨 Submit Report", key="submit_report"):
            ...
            st.success("✅ Report submitted! Go to 📋 Report Log tab to view all reports.")

# ─────────────────────────────────────────────────────────────────────────────
# TAB 2 — PROTECT IMAGE
# ─────────────────────────────────────────────────────────────────────────────
with tab2:
    st.markdown("### 🔒 Add invisible watermark to protect your image")
    uploaded2 = st.file_uploader("Upload image to protect", type=['jpg','jpeg','png'], key="protect")
    secret = st.text_input("Your secret key (remember this!)", value="MY_SECRET_KEY_2024", key="secret")

    if uploaded2 and secret:
        image2 = Image.open(uploaded2)
        st.image(image2, caption="Your Original Image", use_container_width=True)

        if st.button("🔒 Add Watermark"):
            with st.spinner("Adding invisible watermark..."):
                watermarked = add_watermark(image2, secret)
                buf = io.BytesIO()
                watermarked.save(buf, format='PNG')
                buf.seek(0)
            st.success("✅ Watermark added! Download and use this protected image.")
            st.download_button(
                label="⬇️ Download Protected Image",
                data=buf,
                file_name="protected_" + uploaded2.name.split('.')[0] + ".png",
                mime="image/png"
            )
...
# ─────────────────────────────────────────────────────────────────────────────
with tab3:
    st.markdown("### ✅ Verify if an image contains your watermark")
...
# ─────────────────────────────────────────────────────────────────────────────
with tab4:
    st.markdown("### 📋 Report Log — All flagged images")

    if len(st.session_state.reports) == 0:
        st.info("No reports yet. Go to 🔍 Detect Fake tab and report a suspicious image.")
    else:
